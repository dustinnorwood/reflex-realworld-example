...
package = elClass "div" "package-page" $ do
  ...
  (successE,_,_) <- Client.getPackage tokDyn (pure . unDocumentSlug <$> slugDyn) $ pbE <> (void $ updated tokDyn)
...
packageContent packageDyn = prerender_ (text "Rendering Document...") $ do
  let htmlDyn = (\mp -> (maybe "" Package.image mp, maybe "" (markDownToHtml5 . Package.body) mp)) <$> packageDyn
